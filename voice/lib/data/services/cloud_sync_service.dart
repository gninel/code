...
class CloudSyncService {
  ...
  Future<void> sendVerificationCode(String phone) async {
    // 测试模式:直接返回成功,不实际发送验证码
    if (TestConfig.isTestMode) {
      print('测试模式: 验证码为 ${TestConfig.testVerificationCode}');
  ...
  Future<User> registerWithPhone(
    ...
    if (TestConfig.isTestMode) {
      ...
      // 创建测试用户
      _accessToken = 'test_token_${DateTime.now().millisecondsSinceEpoch}';
      _currentUser = User(
        id: 'test_user_$phone',
        email: '$phone@test.com',
        nickname: nickname ?? '用户$phone',
    ...
    // 生产模式:实际调用API
    try {
      final response = await _dio.post('/auth/phone/register', data: {
        'phone': phone,
        'code': code,
        'password': password,
        'nickname': nickname,
