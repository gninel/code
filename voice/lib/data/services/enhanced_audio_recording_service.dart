...
class EnhancedAudioRecordingService {
    ...
    _asrService.recognitionResultStream.listen(
      ...
      onDone: () {
        ...
        if (_isRecording && !_isPaused) {
          // 录音仍在进行，需要重连
          print('RecordingService: Recording still active, reconnecting ASR...');
          _reconnectAsr();
        }
      },
    );
  ...
  Future<void> _reconnectAsr() async {
    ...
    try {
      ...
      if (_audioBuffer.isNotEmpty) {
        print('RecordingService: Flushing ${_audioBuffer.length} buffered audio chunks...');
  ...
  Future<void> recognizeFile(
    ...
    try {
      ...
      print('Service: Starting ASR...');
      // 初始化语音识别
      await _asrService.startRecognition();
      print('Service: ASR started');
      
      onStarted?.call();
      _isRecording = true; 
      _isPaused = false;
      _isFileUpload = true; // 标记为文件上传模式
      _currentRecordingPath = filePath; 
      _recordingStartTime = DateTime.now();

      // 监听识别结果
      final subscription = _asrService.recognitionResultStream.listen((result) {
        print('Service: Received stream data: $result');
        ...
        if (accumulatedText.isNotEmpty) {
          print('Service: Accumulated text: $accumulatedText');
          _accumulatedText = accumulatedText;
          onRecognitionResult?.call(accumulatedText, confidence);
        }
      }, onError: (error) {
        print('Service: Stream error: $error');
        onError?.call(error.toString());
        _isRecording = false;
      });
      ...
      await for (final chunk in fileStream) {
        if (!_isRecording) {
          print('Service: Recording stopped during file read');
          break; 
        }
        ...
        for (var i = 0; i < chunk.length; i += chunkSize) {
          ...
          // print('Service: Sending ${subChunk.length} bytes'); // Too noisy? maybe print every N bytes
          await _asrService.sendAudioData(Uint8List.fromList(subChunk));
          await Future.delayed(const Duration(milliseconds: 20)); 
        }
      }
      print('Service: Total bytes sent: $totalBytes');

      if (_isRecording) {
         print('Service: Sending end frame and waiting for final result...');
         ...
         while (waitCount < maxWait) {
           ...
           waitCount++;
           // 如果收到了新的文本，重置等待计数
           final currentText = _asrService.getFinalText();
           if (currentText.length > _accumulatedText.length) {
             _accumulatedText = currentText;
             onRecognitionResult?.call(_accumulatedText, 0.9);
             waitCount = 0; // 重置等待，继续等可能还有更多
           }
         }
         
         // 获取最终累积的文本
         final finalText = _asrService.getFinalText();
         if (finalText.isNotEmpty && finalText != _accumulatedText) {
           _accumulatedText = finalText;
           onRecognitionResult?.call(_accumulatedText, 0.9);
         }
         print('Service: Final accumulated text: $_accumulatedText');
  ...
  Future<String?> stopRecording() async {
    ...
    try {
      ...
      try {
        ...
        // 获取最终累积的文本
        final finalText = _asrService.getFinalText();
        if (finalText.isNotEmpty && finalText.length > _accumulatedText.length) {
          _accumulatedText = finalText;
          onRecognitionResult?.call(_accumulatedText, 0.9);
        }
      } catch (e) {
  ...
  Future<void> _startRecording() async {
    ...
    _audioStreamSubscription = stream.listen((data) async {
      ...
      // 发送给 ASR
      // 如果正在重连或连接未建立，先缓冲
      if (_isReconnecting || !_asrService.isConnected) {
        _audioBuffer.add(Uint8List.fromList(data));
        return;
      }

      try {
        await _asrService.sendAudioData(Uint8List.fromList(data));
      } catch (e) {
        // 发送失败，加入缓冲
        print('RecordingService: Send failed ($e), buffering data...');
        _audioBuffer.add(Uint8List.fromList(data));

        // 如果是 WebSocket 连接失败，尝试重连
        if (e.toString().contains('WEBSOCKET_CONNECTION_FAILED') || 
            e.toString().contains('WebSocket is closed') ||
            e.toString().contains('invalid handle')) {
          print('RecordingService: ASR connection issue detected: $e');
          // 使用统一的重连逻辑，确保重新挂载监听器
          await _reconnectAsr();
        }
      }
