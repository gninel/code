...
class AiGenerationRepositoryImpl implements AiGenerationRepository {
  ...
  Future<Either<Failure, String>> generateAutobiography({
    ...
    try {
      ...
      final voiceContents = voiceRecords
          .where((record) => record.transcription != null && record.transcription!.isNotEmpty)
          .map((record) => record.transcription!)
          .toList();
  ...
  Future<Either<Failure, String>> generateChapterContent({
    ...
    try {
      ...
      return Left(AiGenerationFailure.contentGenerationFailed(message: e.message));
    } catch (e) {
      return Left(AiGenerationFailure.contentGenerationFailed(message: e.toString()));
    }
  }
