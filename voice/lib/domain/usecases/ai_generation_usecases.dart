...
class AiGenerationUseCases {
  ...
  /// 完整生成自传（包含标题和摘要）
  Future<Either<Failure, AutobiographyGenerationResult>>
    ...
    try {
      ...
      print(
          'AiGenerationUseCases: Content generation took ${contentEnd - contentStart}ms');
      ...
      return contentResult.fold(
        ...
        (content) async {
          ...
          print(
              'AiGenerationUseCases: Title generation took ${titleEnd - titleStart}ms');
          ...
          print(
              'AiGenerationUseCases: Summary generation took ${summaryEnd - summaryStart}ms');

          stopwatch.stop();
          print(
              'AiGenerationUseCases: Total generation took ${stopwatch.elapsedMilliseconds}ms');
  ...
  Future<Either<Failure, IncrementalUpdateResult>>
    ...
    try {
      ...
      return structureResult.fold(
        ...
        (plan) async {
          print(
              'AiGenerationUseCases: Structure plan: ${plan.action}, reason: ${plan.reasoning}');

          // Step 2: 根据计划生成或更新内容
          print(
              'AiGenerationUseCases: Step 2 - Generating/updating content...');
          ...
          switch (plan.action) {
            case StructureAction.createNew:
              // 创建新章节 - 但需要传递完整的现有内容以便 AI 进行时间线融合
              // 优先使用 chapters, 如果为空则使用 content 字段
              String? existingFullContent;
              print(
                  'AiGenerationUseCases: currentChapters.length=${currentChapters.length}');
              print(
                  'AiGenerationUseCases: currentAutobiography is null? ${currentAutobiography == null}');
              if (currentAutobiography != null) {
                print(
                    'AiGenerationUseCases: currentAutobiography.content.length=${currentAutobiography.content.length}');
              }
              if (currentChapters.isNotEmpty) {
                existingFullContent =
                    currentChapters.map((c) => c.content).join('\n\n');
              } else if (currentAutobiography != null &&
                  currentAutobiography.content.isNotEmpty) {
                existingFullContent = currentAutobiography.content;
              }
              print(
                  'AiGenerationUseCases: existingFullContent is null? ${existingFullContent == null}');

              final contentResult = await _repository.generateChapterContent(
                originalContent: existingFullContent, // 传递完整内容以便时间线融合
              ...
              return contentResult.fold(
                (failure) => Left(failure),
                (content) {
                  // 如果使用了 existingFullContent，说明是全量融合（Timeline Integration），
                  // 返回的内容是完整的自传，需要替换现有所有内容。
                  if (existingFullContent != null) {
                    ...
                    updateIndex = 0; // 替换索引为0

                    stopwatch.stop();
                    print(
                        'AiGenerationUseCases: Incremental update (Full Replacement) took ${stopwatch.elapsedMilliseconds}ms');

                    return Right(IncrementalUpdateResult(
                      updatedChapter: updatedChapter!,
                      updateType: UpdateType.fullReplacement, // 新增类型：全量替换
                      updateIndex: updateIndex,
                    ));
                  }

                  final newOrder =
                      currentChapters.isEmpty ? 0 : currentChapters.length;
                  updatedChapter = Chapter(
                    id: DateTime.now().millisecondsSinceEpoch.toString(),
                  ...
                  stopwatch.stop();
                  print(
                      'AiGenerationUseCases: Incremental update took ${stopwatch.elapsedMilliseconds}ms');
