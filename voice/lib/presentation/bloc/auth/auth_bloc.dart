...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(
    ...
    on<PhoneRegister>(_onPhoneRegister);
  ...
  Future<void> _onInitAuth(InitAuth event, Emitter<AuthState> emit) async {
    try {
      ...
      if (_cloudSyncService.isLoggedIn) {
        // 启用自动同步
        _autoSyncService.enableAutoSync();
        emit(state.copyWith(
  ...
  Future<void> _onSendVerificationCode(
    ...
    try {
      await _cloudSyncService.sendVerificationCode(event.phone);
      // 验证码发送成功，不改变认证状态
    } catch (e) {
      emit(state.copyWith(
        error: e.toString(),
  ...
  Future<void> _onPhoneRegister(
      PhoneRegister event, Emitter<AuthState> emit) async {
    emit(state.copyWith(status: AuthStatus.loading, error: null));

    try {
      final user = await _cloudSyncService.registerWithPhone(
        event.phone,
        event.code,
  ...
  Future<void> _onPhoneLogin(
      PhoneLogin event, Emitter<AuthState> emit) async {
    emit(state.copyWith(status: AuthStatus.loading, error: null));

    try {
      final user = await _cloudSyncService.loginWithPhone(
        event.phone,
        event.code,
      );
      // 启用自动同步
  ...
  Future<void> _onLogout(Logout event, Emitter<AuthState> emit) async {
    // 禁用自动同步
    _autoSyncService.disableAutoSync();
